...
| BinaryExpr (left, operator, right) -> Printf.sprintf "Binary(%s, %s, %s)" (string_of_expression left) (Token.string_of_token operator) (string_of_expression right)
...
| UnaryExpr (operator, right) -> Printf.sprintf "Unary(%s, %s)" (Token.string_of_token operator) (string_of_expression right)